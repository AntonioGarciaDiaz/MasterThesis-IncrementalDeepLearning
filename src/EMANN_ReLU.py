...
EMANN algorithm, except it uses the Rectified Linear Unit (ReLU) as an
activation function (instead of sigmoid) for the hidden layer, and the
...
# The activation function slope for leaky ReLU.
ALPHA = 0.2
...
def ReLU_prime(x):
    '''
    The derivative of the "leaky" Rectified Linear Unit (ReLU) operation.

    ReLU_prime(x) = ALPHA if x <= 0      ReLU_prime(x) = 1 if x > 0
    '''

    greater_than = tf.cast(tf.greater(x, np.zeros_like(x)), tf.float32)
    less_than = tf.cast(tf.less(x, np.zeros_like(x)), tf.float32)
    ReLU_prime = tf.add(greater_than, tf.scalar_mul(ALPHA, less_than))
    return ReLU_prime
...
class Module:
    ...
    def __init__(self, module_ID, input_channel, input_units, hidden_units,
        ...
        # N.B.: The activation function is ReLU (value between 0 and infinite).
        self.z_1 = tf.matmul(self.a_0, tf.stack(self.w_1, axis=1))
        self.a_1 = tf.nn.leaky_relu(self.z_1, alpha=ALPHA)
    ...
    def add_new_unit(self, sess, unit_count, reg_constant=0):
        ...
        print("Added", unit_count, "units! Number of units:", self.a_1_units)
